...
def parse_args():
    ...
    parser.add_argument(
        ...
        '-sp',
        type=str,
        default="my_projects/test_results/images_plots/prediction_images"
    )
    parser.add_argument(
        '--set_name',
        '-sn',
    ...
    if not args.set_name:
        args.set_name = f"{args.dataset_name}_{args.pred_name}".replace(
            ".png", ""
        )
...
def get_gt_img(args):
    gt_path = os.path.join(
        DATASET_TEST_ANN_PATH[args.dataset_name],
        args.pred_name
    )
    try:
...
def get_pred_label(args, model_dir_path):
...
def get_rgb_img(args):
    rgb_path = os.path.join(
        DATASET_TEST_IMG_PATH[args.dataset_name],
        args.pred_name
    )
    try:
...
def get_model_pred_labels(args):
    pred_dict = {}
    for model_dir_name in os.listdir(args.results_path):
        ...
        pred_label = get_pred_label(args=args, model_dir_path=model_dir_path)
        pred_dict[model_name] = pred_label
    return pred_dict

def get_all_figures_dict(args):
    pred_dict = get_model_pred_labels(args=args)
    pred_dict["Ground Truth"] = get_gt_img(args=args)
    return pred_dict
...
def generate_set_figure(args):
    ...
    p_utils.set_params(param_dict=p_utils.PRED_VISUALIZATION_FIGURE_PARAMS)
